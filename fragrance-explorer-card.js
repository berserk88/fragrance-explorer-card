(function () {
  ...
  const DB = [
    ...
    { id:16, name:"Spiced Chai & Tobacco",       icon:"🌶️",rating:4.5, season:"Autumn",      time:"All",   occasion:"Casual",  profile:"Ultra-cozy spiced gourmand.",                                        synergy:"Hercules injects warm cinnamon spice.",                          fragrances:["Hercules","Immortal"],                                                  steps:[{f:"Hercules",v:"1 spray",z:"Neck"},{f:"Immortal",v:"1 spray",z:"Chest"}]},
    ...
    { id:24, name:"Smoky Mountain Vetiver",      icon:"⛰️",rating:4.0, season:"Autumn",      time:"Day",   occasion:"Office",  profile:"Dark ink vetiver meeting clean tea.",                                synergy:"Heaven provides clean fresh air.",                               fragrances:["Heaven","Encre Noire"],                                                 steps:[{f:"Heaven",v:"1 spray",z:"Wrists"},{f:"Encre Noire",v:"2 sprays",z:"Neck"}]},
    ...
    { id:26, name:"Aromatic Ink & Leather",      icon:"📖", rating:4.5, season:"Winter",      time:"Night", occasion:"Formal",  profile:"Intense ultra-masculine dark niche wood.",                           synergy:"Pure dark art excellence.",                                      fragrances:["Encre Noire","Ghost"],                                                  steps:[{f:"Encre Noire",v:"2 sprays",z:"Chest"},{f:"Ghost",v:"1 spray",z:"Neck"}]},
    ...
    { id:28, name:"Golden Pineapple Breeze",     icon:"☀️", rating:4.5, season:"Summer",      time:"Day",   occasion:"Casual",  profile:"Premium sharp tropical pineapple.",                                  synergy:"Divin Asylum supercharges fruity notes.",                        fragrances:["Divin Asylum","Immortal"],                                              steps:[{f:"Divin Asylum",v:"2 sprays",z:"Neck"},{f:"Immortal",v:"1 spray",z:"Chest"}]},
    ...
    { id:34, name:"Sovereign Niche Overlord",    icon:"🗡️",rating:5.0, season:"Winter",      time:"Night", occasion:"Formal",  profile:"[4-LAYER] Ultimate niche symphony.",                                 synergy:"Heavy, dense base elements shifting smoothly into sweet gourmand top.", fragrances:["Encre Noire l'Extreme","Costume National I","Lattafa Liam Grey","Spectre Ghost"], steps:[{f:"Encre Noire l'Extreme",v:"1 spray",z:"Skin / Lower Back"},{f:"Costume National I",v:"2 sprays",z:"Skin / Chest"},{f:"Lattafa Liam Grey",v:"1 spray",z:"Skin / Neck"},{f:"Spectre Ghost",v:"1 spray",z:"Hair / Shoulders"}]},
  ];

  const SEASONS   = [{l:"Spring",i:"🌸"},{l:"Summer",i:"☀️"},{l:"Autumn",i:"🍂"},{l:"Winter",i:"❄️"},{l:"All Seasons",i:"🌍"}];
  const TIMES     = [{l:"Day",i:"🌤️"},{l:"Night",i:"🌙"},{l:"All",i:"🕐"}];
  const OCCASIONS = [{l:"Casual",i:"👟"},{l:"Office",i:"💼"},{l:"Evening",i:"🍷"},{l:"Formal",i:"🎩"}];
  ...
  const CSS = `
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .shell{
      ...
      height:var(--fh,400px);max-height:var(--fh,400px);
    ...
    /* search */
    .sb{flex-shrink:0;padding:10px 12px 8px;background:var(--sidebar-background-color,#111);border-bottom:1px solid var(--divider-color,rgba(255,255,255,.08))}
    .si{width:100%;padding:8px 12px;border-radius:8px;border:1.5px solid var(--divider-color,rgba(255,255,255,.15));background:var(--secondary-background-color,#2c2c2e);color:var(--primary-text-color,#fff);font-size:13px;font-family:inherit;outline:none;-webkit-appearance:none}
    .si:focus{border-color:var(--accent-color,#e07b54)}
    .si::placeholder{color:var(--secondary-text-color,#888)}
    /* body */
    .bd{flex:1;overflow-y:auto;overflow-x:hidden;padding:12px;-webkit-overflow-scrolling:touch}
    /* nav */
    .nv{flex-shrink:0;display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:var(--sidebar-background-color,#111);border-top:1px solid var(--divider-color,rgba(255,255,255,.08));min-height:44px}
    .ni{font-size:12px;color:var(--secondary-text-color,#888);font-weight:500}
    .nb{display:flex;gap:8px}
    /* buttons */
    button{font-family:inherit;cursor:pointer;-webkit-tap-highlight-color:transparent;border:none;line-height:1}
    .bb{padding:6px 14px;border-radius:6px;font-size:12px;font-weight:600}
    .bk{background:var(--secondary-background-color,#2c2c2e);color:var(--primary-text-color,#fff)}
    .bh{background:var(--accent-color,#e07b54);color:#fff}
    /* section */
    .sl{font-size:10px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:var(--accent-color,#e07b54);margin:0 0 8px}
    .sc{margin-bottom:14px}
    /* chips */
    .cr{display:flex;flex-wrap:wrap;gap:7px}
    .ch{display:inline-flex;align-items:center;gap:5px;padding:6px 11px;border-radius:20px;border:1.5px solid var(--divider-color,rgba(255,255,255,.15));background:var(--secondary-background-color,#2c2c2e);color:var(--primary-text-color,#e0e0e0);font-size:12px;font-weight:600;cursor:pointer;user-select:none}
    .ch.on{background:var(--accent-color,#e07b54);border-color:var(--accent-color,#e07b54);color:#fff}
    /* list */
    .ls{display:flex;flex-direction:column;gap:9px}
    .lc{display:flex;align-items:flex-start;gap:10px;padding:11px 12px;border-radius:10px;background:var(--secondary-background-color,#2c2c2e);border:1px solid var(--divider-color,rgba(255,255,255,.07));cursor:pointer}
    .lc:active{opacity:.75}
    .le{font-size:24px;flex-shrink:0;margin-top:2px;line-height:1}
    .li{flex:1;min-width:0}
    .ln{font-size:13px;font-weight:700;color:var(--primary-text-color,#fff);margin-bottom:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .lt{display:flex;flex-wrap:wrap;gap:4px}
    .tg{font-size:10px;font-weight:600;padding:2px 7px;border-radius:4px;background:rgba(224,123,84,.15);color:var(--accent-color,#e07b54);white-space:nowrap}
    .lsr{flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:2px}
    .ss{font-size:12px;color:#f5c518;letter-spacing:1px;white-spac
